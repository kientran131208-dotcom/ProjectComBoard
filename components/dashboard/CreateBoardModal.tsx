...
export default function CreateBoardModal({ 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(error);
      alert(t.modals.createBoard.error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      {/* Semi-transparent Overlay - NO BLUR as requested */}
      <div 
      ...
      <div className="relative w-full max-w-[580px] bg-white border-2 border-black rounded-xl shadow-[8px_8px_0px_#000000] flex flex-col max-h-[90vh] z-[110] overflow-hidden">
        ...
        <div className="p-6 border-b-2 border-black bg-[#f2efff] rounded-t-xl relative">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="font-headline font-bold text-2xl text-[#2d2d42] flex items-center gap-2">
                <span className="text-3xl">🎉</span> {t.modals.createBoard.title}
              </h2>
              <p className="text-sm text-[#5a5971] mt-1 font-medium">{t.modals.createBoard.subtitle}</p>
            </div>
            <button 
              onClick={onClose}
          ...
          <div className="absolute bottom-0 left-6 right-6 h-1 opacity-20 bg-repeat-x" style={{ backgroundImage: "url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMCIgaGVpZ2h0PSI0Ij48cGF0aCBkPSJNMCAwbDUgNCA1LTQiIGZpbGw9Im5vbmUiIHN0cm9rZT0iI2Y5Y2M2MSIgc3Ryb2tlLXdpZHRoPSIyIi8+PC9zdmc+')" }}></div>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-8 bg-[#f8f5ff] bg-polka-dots">
              ...
              <div>
                ...
                <div className="flex gap-4">
                  {colors.map((c) => (
                    ...
                    >
                      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0IiBoZWlnaHQ9IjQiPjxjaXJjbGUgY3g9IjIiIGN5PSIyIiByPSIxIiBmaWxsPSJyZ2JhKDAsMCwwLDAuMSkiLz48L3N2Zz4=')]"></div>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-[#2d2d42] mb-2 text-center sm:text-left">{t.modals.createBoard.tagInputLabel}</label>
                    ...
                    formData.tags.map((tag) => (
                      ...
                      >
                        ...
                        <button 
                          ...
                          className="w-4 h-4 rounded-full flex items-center justify-center hover:bg-black/20"
                        >
                ...
                <div className="p-4 bg-[#f9cc61]/10 border-2 border-black border-dashed rounded-lg">
                    <span className="text-[#b71212]">{t.modals.createBoard.tagHint}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              <div className="p-6 bg-[#dbd9ff] border-2 border-black rounded-lg">
                <h4 className="font-bold text-sm mb-2 flex items-center gap-2">
                  <span>📨</span> {t.modals.createBoard.inviteTitle}
                </h4>
                <p className="text-xs text-[#2d2d42] leading-relaxed">{t.modals.createBoard.inviteDesc}</p>
              </div>
            </div>
          )}
        ...
        <div className="p-6 border-t-2 border-black bg-[#f2efff] shrink-0 flex justify-between items-center rounded-b-xl">
          ...
          >
            {loading ? t.modals.createBoard.creating : step === 3 ? t.modals.createBoard.finish : t.modals.createBoard.next}
