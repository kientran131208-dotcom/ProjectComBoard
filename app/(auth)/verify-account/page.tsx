...
import { verifyRegistrationAction } from "@/lib/actions/authActions";
export const dynamic = "force-dynamic";

export default async function VerifyAccountPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
    ...
    <div className="flex min-h-screen w-full flex-col lg:flex-row antialiased">
      ...
      <div className="relative hidden lg:flex w-1/2 flex-col justify-between overflow-hidden bg-[#1A1A2E] bg-polka-white p-12">
        ...
        <div className="z-10 mt-auto">
          ...
          <p className="mt-6 font-inter text-base text-white/70">Xác minh tài khoản của bạn để bắt đầu kết nối</p>
        </div>
      </div>
      ...
      <div className="flex w-full lg:w-1/2 flex-col justify-center bg-[#FFF8F6] bg-polka-dark px-6 py-12 sm:px-12 lg:px-24">
        <div className="mx-auto w-full max-w-[400px]">
          <div className="mb-10 text-left">
            <h2 className="font-poppins text-3xl font-bold text-[#1A1A2E]">Xác thực tài khoản 🛡️</h2>
            <p className="mt-2 font-inter text-sm text-[#5a5971]">Mã xác thực 6 chữ số đã được gửi đến <span className="font-bold text-black">{email}</span></p>
          </div>
          ...
          {error === "ExpiredCode" && (
            <div className="mb-6 p-4 bg-[#FFD166] border-2 border-black text-black font-bold text-xs shadow-[4px_4px_0_0_#000]">
               ⏳ Mã đã hết hạn. Vui lòng kiểm tra hòm thư hoặc đăng ký lại.
            </div>
          )}
          
          <form action={verifyRegistrationAction} className="space-y-6">
          ...
          <div className="mt-8 text-center space-y-4">
             <p className="text-xs text-[#75748d]">Sử dụng email thật để nhận mã xác thực. Hệ thống tự động chặn các dịch vụ email rác.</p>
